...
**训练过程的对比可视化（P3/P4 必做，不止最终曲线）**：训练 Dyna-Q 时不要只在跑完后画一张静态学习曲线，而要做一个**把"有世界模型(K=10)"与"无世界模型(K=0)"连在一起、能看出训练过程中差距的对比可视化**，目标是一眼看出"**在同样的真实步数 / 同一轮次下，有世界模型让 agent 变好了还是变坏了**"：

- **共享 x 轴对齐**：两条（或多条）曲线必须用**同一个真实步数横轴**叠在同一张图里（K=0 vs K=10，外加 VI 天花板虚线），这样"第 N 步时谁高谁低"是直接可读的——这正是 Figure 3 的核心，但要在训练**过程中**就增量画出来、边训边看，而不是只在最后出图。
- **过程动画优先**：做一个随训练推进的动图/逐帧图（如每 500/2000 真实步存一帧），让两条学习曲线**同步生长**，或并排显示两个 agent 在同一组测试 episode 上的贪婪策略行为（用 `animate_episode(env, policy=greedy)`），直观对比"同一轮次，有模型的轨迹更快到终点 / 或被坏模型带偏"。
- **差距量化标注**：在对比图上标出关键差距，比如"达到 50% 成功率所需真实步数：K=10 vs K=0"，或在固定步数处两者成功率的差值（Δ），把"快了多少 / 坏模型差了多少"写成数字。
- **正反都要可见**：这个可视化既要能展示 P4 里**好模型加速**（K=10 更早爬升），也要能展示 P5 里**坏模型拖累**（200 条模型的曲线被想象更新拉低甚至低于 K=0），即"预测错了会怎样"。

可视化工具一览（`plotting.py`）：`plot_grid_layout`（静态布局）、`animate_episode`（GIF 动图）、`plot_visitation_heatmap`（Figure 1 访问热力图）、`plot_transition_accuracy`（Figure 2 准确率+top-3 预测）；P3/P4 待加：学习曲线对比图 + 训练过程动画。
